...
def prepare_dataset(batch):
    ...
    batch["input_features"] = feature_extractor(audio["array"], sampling_rate=audio["sampling_rate"]).input_features[0].astype(np.float16)
    del batch["audio"]
...
if __name__ == '__main__':
    ...
    for run_fold in range(5):
        ...
        train_ds = load_to_dataset(
            audio_file_paths=audio_file_paths[:fold * fold_size] + audio_file_paths[(fold + 1) * fold_size:],
            tokens=np.concatenate([tokens[:fold * fold_size], tokens[(fold + 1) * fold_size:]]),
            decoder_mask=np.concatenate([masks[:fold * fold_size], masks[(fold + 1) * fold_size:]]),
            target=np.concatenate([labels[:fold * fold_size], labels[(fold + 1) * fold_size:]])
        )
        ...
        training_args = TrainingArguments(output_dir=f"../weights/oof-{args.model_name.split('-')[1]}-epoch_{args.epochs}/fold_{run_fold}/",
